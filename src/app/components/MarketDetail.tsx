...
export function MarketDetail({ market, onClose }: MarketDetailProps) {
  ...
  useEffect(() => {
    ...
    if (Number.isFinite(cached)) {
      ...
      setStartPrice((prev) => (prev == null ? cached : prev));
      setPoints((prev) => {
        ...
        for (let i = 20; i >= 1; i--) seed.push({ t: now - i * 1000, p: cached });
        return seed;
      });
    } else {
      // seed with a placeholder so the chart is visible immediately
      setPoints((prev) => {
          // replace placeholder zeros on first real tick
          if (prev.length && prev.every(x => x.p === 0)) return [{ t: Date.now(), p }];
        if (prev.length) return prev;
        const now = Date.now();
        const seed: QuickPoint[] = [];
        for (let i = 20; i >= 1; i--) seed.push({ t: now - i * 1000, p: 0 });
        return seed;
      });
    }
  ...
  if (!isQuick) {
    ...
    return (
      <div className="min-h-screen bg-[#050B1A] pb-24">
  ...
  return (
    <div className="min-h-screen bg-[#050B1A] pb-28">
      <div className="px-4 py-4 flex items-center justify-between border-b border-white/10">
      ...
      <div className="px-4 py-6 space-y-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            ...
            <div className="text-xs text-white/60 mt-1">Implied {Math.round(odds.pUp * 100)}%</div>
          </button>
          ...
          >
            ...
            <div className="text-xs text-white/60 mt-1">Implied {Math.round(odds.pDown * 100)}%</div>
          </button>
        </div>
