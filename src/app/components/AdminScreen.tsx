...
export function AdminScreen({ adminAddress, activeAddress, network, onNetworkChange, onClose }: Props) {
  ...
  const onTopUp = () => {
    ...
    // Store credits by username (demo). Later replaced with DB + contract.
    const key = `qmvp_demo_credits_username_${u.toLowerCase()}`;
    const cur = Number(localStorage.getItem(key) || "0");
    localStorage.setItem(key, String(cur + amount));
    setStatus(`Added $${amount} demo credits to @${u}`);
    setUsername("");
  };
    ...
    <div className="fixed inset-0 z-[60] bg-[#050B1A]">
      <div className="px-4 pt-6 pb-24">
        ...
        <div className="mt-5 rounded-2xl border border-white/10 bg-gradient-to-br from-[#0B1B44]/40 to-[#050B1A] p-4">
          <p className="text-white font-medium">Team demo credits</p>
          ...
          <div className="mt-4 grid grid-cols-1 gap-3">
            <input
              ...
              className="h-11 rounded-xl bg-[#071027] border border-white/10 px-3 text-white placeholder:text-white/30 outline-none focus:border-cyan-400/40"
            />
            <input
              value={credits}
